...
object UTry:
  ...
  extension UTryOps on [A](self: UTry[A]):
    ...
    def failed: UTry[Throwable] = self.fold(e => e.wrap)(_ => NoSuchElementException("Success"))
  ...
  extension UTryMapOps on [A, B](self: UTry[A]):
    def map(f: A => B): UTry[B] = self.fold(e => e)(a => f(a).wrap)
    def flatMap(f: A => UTry[B]): UTry[B] = self.fold(e => e)(a => f(a))
    def transform(s: A => UTry[B], f: Throwable => UTry[B]): UTry[B] =
      self.fold(f)(s)

  extension UTryFoldOps on [A, B](self: UTry[A]):
    def fold(fe: (Throwable) => B, fa: (A) => B): B = impl.fold(self)(fe)(fa)
